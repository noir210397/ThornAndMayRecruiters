        ...
        try {
            await session.withTransaction(async () => {
                ...
                await Shift.findByIdAndUpdate(
                    shiftId,
                    { $inc: { agentsBooked: agents.length, availableSlots: -agents.length } },
                    { session, new: true }
                );
...
export const updateShift = async (newShiftValues: UpdateShiftRequest, shiftId: string) => {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (newAgents && newAgents.length > 0) {
                ...
                if (booked.length > 0) throw new CustomError(StatusCode.Status403Forbidden, null, "booking same agent multiple times for same shift is not allowed")
...
export async function bookSingleAgent(agentId: string, shiftId: string) {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (!shift) throw new CustomError(StatusCode.Status404NotFound, null, `no shift with id ${shiftId} found`)
            const booked = await Booking.exists({ agentId: agentId, shiftId: shiftId }).session(session)
            if (booked) throw new CustomError(StatusCode.Status403Forbidden, null, "this agent is alreay booked for this shift")
            ...
            else throw new CustomError(StatusCode.Status403Forbidden, null, `there are currently only ${shift.agentsRequired - shift.agentsBooked} slots remaining`)
        })

    } catch (err: unknown) {
        if (err instanceof CustomError) throw err
        else throw new CustomError(StatusCode.Status500ServerError, null, "unable to update shift")
...
export async function deleteShift(shiftId: string, withBookings: boolean) {
    const session = await mongoose.startSession()
    try {
        await session.withTransaction(async () => {
            const shift = await Shift.findById({ _id: shiftId }).lean()
            if (!shift) throw new CustomError(StatusCode.Status404NotFound, null, `no shift found with Id:${shiftId}`)
            if (withBookings) {
        ...
        if (err instanceof CustomError) throw err
        else throw new CustomError(StatusCode.Status500ServerError, null, `unable to delete ${withBookings ? "shift along with its bookings" : "shift"}`)
    }
    finally { await session.endSession() }

}
export async function getShifts(user: AccessTokenPayload, query: BaseQuery) {
    const { page } = query
...
export async function getSingleShift(shiftId: string, user: AccessTokenPayload,) {
    if (user.role === Role.AGENT.toString()) {
        const today = new Date()
        const shift = await Shift.findById(shiftId, null, { isPublic: true, startTime: { $gte: today } }).lean()
        if (!shift) throw new CustomError(StatusCode.Status404NotFound, null, "shift not found")
